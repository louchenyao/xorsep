...
static void BM_build_native_(benchmark::State& state) {
    benchmark_build_function(state, HashGroup::build_naive_<uint64_t, MixFamily<uint64_t>>);
}
BENCHMARK(BM_build_native_);

static void BM_build_bitset_(benchmark::State& state) {
    benchmark_build_function(state, HashGroup::build_bitset_<uint64_t, MixFamily<uint64_t>>);
}
BENCHMARK(BM_build_bitset_);

static void BM_build_bitset_2_(benchmark::State& state) {
    benchmark_build_function(state, HashGroup::build_bitset_2_<uint64_t, MixFamily<uint64_t>>);
}
